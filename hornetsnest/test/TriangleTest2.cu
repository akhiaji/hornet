...
int hostSingleIntersection (const vert_t ai, const degree_t alen, const vert_t * a,
                            const vert_t bi, const degree_t blen, const vert_t * b){
    ...
    const vert_t *aptr=a, *aend=a+alen;
    const vert_t *bptr=b, *bend=b+blen;
...
void hostCountTriangles (const vert_t nv, const vert_t ne, const eoff_t * off,
    const vert_t * ind, int64_t* allTriangles)
{
    //int32_t edge=0;
    int64_t sum=0;
    for (vert_t src = 0; src < nv; src++)
    {
        degree_t srcLen=off[src+1]-off[src];
        for(int iter=off[src]; iter<off[src+1]; iter++)
        {
            vert_t dest=ind[iter];
